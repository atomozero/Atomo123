...
static status_t ReadASCD(BPositionIO* source, CContainer* doc)
{
	...
	// SEPARATO da kASCDVersion apposta, non lo stesso valore. Questa
	// ReadASCD non legge MAI la sezione formattazione condizionale (si
	// ferma subito dopo le celle, vedi sotto), quindi ogni nuovo campo
	// per-regola introdotto in una versione successiva (3: punti di
	// scala di colori; 4: riferimento di cella per il confronto) non
	// serve saltarlo qui: basta accettare la versione, non serve altro.
