...
- Real sheet-protection passwords: "Proteggi foglio" was an
  ...
  reusable).
- Workbook open-password decryption (Agile Encryption, read-only): a real
  password-protected `.xlsx` (Excel 2010+, the "encrypt with password"
  option) used to fail to open at all — it isn't a ZIP, it's an OLE2/CFB
  container (`EncryptionInfo` + `EncryptedPackage` streams), and
  `CZipReader::Open` just rejected it outright. Opening one now prompts
  for the password (reusing `PasswordWindow` from the sheet-protection
  work above) and, once verified, decrypts the real ZIP/OOXML bytes into
  memory and feeds them into the normal `Identify`/`Translate` pipeline —
  the translator itself never learns the file was ever encrypted.
  Read-only by design (re-exporting drops the encryption, same as the
  sheet-protection hash toward external tools); a plain "Salva" on a
  decrypted document now warns before silently overwriting the encrypted
  original with a plaintext copy. Scoped to Agile Encryption only (the
  legacy 2007-2010 "Standard Encryption" binary format is out of scope —
  Agile covers the overwhelming majority of real protected files). New
  `CompoundFileReader` (`engine/src/FileSys/`), a general OLE2/CFB reader
  (header + DIFAT + FAT + MiniFAT + directory tree, by-name stream
  lookup) — deliberately separate from the existing legacy
  `Excel.OLE2.cpp` (BIFF8-only, hardcoded to the `Workbook`/`Book`
  stream), zero shared risk with XLS import. New `OfficeCrypto`
  (`engine/src/FileSys/`) implements the MS-OFFCRYPTO Agile Encryption
  key derivation/verification/package decryption, reusing the same
  iterated-SHA512 core as the sheet-protection hash (different "block
  key" constants). Verified against a real, spec-conformant test fixture
  (`translators/xlsx/tests/sample_encrypted.xlsx`) generated by an
  independent Python reference implementation (`pycryptodome`), not by
  this project's own code — both the correct-password and wrong-password
  paths are covered end-to-end through the real interactive
  `MainWindow::OpenFileAsync` path, not just the algorithm in isolation.
  Found and fixed a real bug while writing this: two spots in the key/IV
  derivation were reusing `IteratedSha512` (which UTF-8→UTF-16LE-converts
  its input, correct for an actual password) on raw binary data instead,
  silently corrupting the derived keys — split out a plain `Sha512Raw`
  helper for the raw-byte cases.
- Fixed a real regression from the sheet-protection password-hash change
  above: `kASCDVersion` (native `.ascd` format version) went from 8 to 9,
  but the CSV and ODS translators' own separate `kASCDMaxReadableVersion`
  constants (each translator duplicates its own minimal `ReadASCD`, per
  this project's own translator-isolation convention) were left at 5 —
  exporting to `.csv`/`.ods` silently wrote a 0-byte file instead of
  failing loudly. Same bug class as a previous "ASCD version bump broke
  CSV/ODS export" fix, not applied to this particular bump. Found by
  `ui/tests/test_export_formats.cpp` after reinstalling all four
  translators against the rebuilt engine.
