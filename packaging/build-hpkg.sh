...
# passare da haikuporter/atomo123-0.3.0.recipe: quella recipe richiede
...
#   cp atomo123-0.3.0-1-x86_64.hpkg ~/config/packages/   # solo per l'utente corrente
# oppure, per tutto il sistema (richiede privilegi):
#   cp atomo123-0.3.0-1-x86_64.hpkg /boot/system/packages/
# packagefs lo monta automaticamente, senza bisogno di riavviare.
#
# Architettura secondaria a 32 bit (x86, non ancora verificata su
# questo repository -- il toolchain non e' installato qui): una volta
# disponibile (pacchetti gcc_x86/gcc_syslibs_x86 dal repository
# HaikuPorts, nome del compilatore da confermare quando esistono
# davvero, es. x86-unknown-haiku-g++), basta
#   ARCH=x86 CXX=x86-unknown-haiku-g++ packaging/build-hpkg.sh
# I tre Makefile (engine/ui/translators/*) sono gia' pronti a
# ricevere CXX/CXXFLAGS/AR sovrascritti (vedi engine/Makefile) --
# questo script si limita a passare ARCH nel nome del pacchetto/file e
# nella ricetta .PackageInfo, seguendo la convenzione reale di Haiku
# per un pacchetto di architettura secondaria (suffisso "_x86" sul
# nome, "requires { haiku_x86 }" invece di "haiku").
...
if [ "$ARCH" = "x86_64" ]; then
	PKG_NAME="atomo123"
	REQUIRES_HAIKU="haiku"
else
	PKG_NAME="atomo123_$ARCH"
	REQUIRES_HAIKU="haiku_$ARCH"
fi
OUT="$ROOT/packaging/$PKG_NAME-$VERSION-$ARCH.hpkg"
WORKDIR="$ROOT/packaging/hpkg-root"

echo "== Ricompilazione (engine, quattro translator, UI) per $ARCH =="
make -C engine
