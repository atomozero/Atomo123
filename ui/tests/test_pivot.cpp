/*
	...
	WritePivotTable scriva il risultato atteso nel foglio.
*/
...
int main()
{
	...
	if (rows.size() == 2)
	{
		// Ordinato per categoria: Mela prima di Pera.
		Check(rows[0].category == "Mela" && rows[0].aggregate == 30.0
				&& rows[0].count == 2,
			"Mela: somma 30 (10+20), conteggio 2");
		Check(rows[1].category == "Pera" && rows[1].aggregate == 5.0
				&& rows[1].count == 1,
	...
	// (categoria, valore), deve fallire esplicitamente.
